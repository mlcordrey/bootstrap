...
function hover_menu() {
	jQuery(".nav li.sub-menu").hover(function() {
		jQuery(this).find("ul").addClass("submenu_h");
	}, function(){
		jQuery('.nav li.sub-menu ul').removeClass("submenu_h");
	});
...
jQuery(document).ready(function() {
	...
	if (jQuery('.fixed-menu').size() && fixed_menu == true) {
		...
		jQuery('.fixed-menu').append('<div class="fixed-menu-wrapper">'+jQuery('.menu_block').html()+'</div>');
...
function scrolled_menu() {
	if (jQuery(window).scrollTop() > jQuery('.menu_block').height() -28) {
